...
public class Tools3d {
	...
	/**
	 * Creates a rotation matix that will rotate the given point so that it lies
	 * on the x axis. We do the transformation in two steps. First, rotate about
	 * z axis ( so the point lies in the plane y = 0). Second, rotate about y
	 * axis ( so the point lies in the plane z = 0).
	 */
	...
	public static final void applyTo(float[][] m, float[] a, float[] a_) {
		...
		/**
		 * Bug !! if a and a_ refer to the same array we always get zero ! Soln:
		 * introduce a__
		 */
		for (int i = 0; i < 3; i++) {
			for (int j = 0; j < 3; j++) {
	...
	/**
	 * Projects (x, y, z) to (x_, y_, z_). We are doing a perspective projection
	 * with the eye located a distance d along the x axis looking towards the
	 * origin. Return false if (x, y, z) falls outside field of view, true
	 * otherwise.
	 */
	...
	/**
	 * Sets c equal to a minus b. Thus c becomes the vector that takes you from
	 * b to a.
	 */
	...
	/**
	 * Makes c equal the cross product of a and b. The result has magnitude
	 * |a|.|b|.sin(theta) and its direction is perpendicular to the plane
	 * defined by a and b. If, as we look at the plane, we turn clockwise to go
	 * from a to b then c is pointing away from us.
	 * 
	 * <pre>
	 * 
	...
	public static final double rnd(Random random, double lower, double upper) {
		return (double) (random.nextDouble() * (upper - lower) + lower);
	}

	public static final float rnd(Random random, float lower, float upper) {
		return (float) (random.nextFloat() * (upper - lower) + lower);
	}
	...
	/**
	 * SPEED - round theta by splitting PI/4 into N steps. Then look up a pre
	 * computed value from array.
	 */
	...
	private static void initSines() {
		dAngle = (float) (Math.PI / 4) / NUM_ANGLES; // split a right angle into
														// N small steps;
	...
	/**
	 * Prints out the tokens in a stream. Useful for debugging. This class has
	 * become the home of static utility methods !
	 */
	public static void debugTokens(StreamTokenizer st) throws IOException {
		...
		while (st.nextToken() != StreamTokenizer.TT_EOF) {
			...
			case StreamTokenizer.TT_WORD:
				Log.i("FC", "Word: " + st.sval);
				break;
			case StreamTokenizer.TT_NUMBER:
				Log.i("FC", "Num: " + st.nval);
				break;
			...
			case ',':
				...
				break;
			default:
				Log.i("FC", "Ttype, sval: " + st.ttype + ", " + st.sval);
				break;
			}
		}
	}
	...
	public static String asString(float[] p) {
		return "(" + Tools3d.round(p[0]) + ", " + Tools3d.round(p[1]) + ", " + Tools3d.round(p[2]) + ")";
	}

 ...
 * 
 * <p>
 * We keep an extra list of points which represent the object's co-ords in its *local frame* of reference. We dictate that the object's initial co-ords are its
 * local co-ords.
 * 
...
public class Obj3dDir extends Obj3d {
	...
	/**
	 * Creates a copy of an instance of Obj3dDir. The Glider uses this constructor; it is passed an instance of GliderType which holds a 3d object and clones
	 * that object so it has its own copy.
	 */
	...
	/**
	 * Sets the i, j and k vectors so that v points along the j and i and k are banked over. v should be a unit vector and the roll is in radians.
	 * 
	 * <p>
	 * Looking along j (which equals v) towards origin we see the how the roll moves k away from the vertical and i away from the horizontal:
	 * 
	 * <pre>
	 *                       k_    k
	 * 		      |   /
	...
	public void setFrame(float[] p, float[] v, float roll) {
		...
		if (roll == _roll && j[0] == v[0] && j[1] == v[1] && j[2] == v[2]) {
	...
	/**
	 * Updates the gloabl co-ords if the local frame has been rotated. This also handles any translation of the origin.
	 */
	...
	/**
	 * Updates the gloabl co-ords if the local frame has been translated. We have this seperate routine to handle the case where there has been a translation
	 * but no rotation *for speed*. Note that many moving bodies spend much of their time travelling in straight lines.
	 */

...
class NodeManager {
	...
	static final int UPFRONT_NODES = 2; // how many nodes are awake at the same
										// time ?
	...
	private void carveUpTask() {
		...
		while (next < numNodes) {
			...
			next++;
			d += nodeSpacing;
			// if (d >= tp.distanceFromStart + tp.distanceToNext && tp.nextTP !=
			// null) {
		...
		if (task.hills != null)
			for (int i = 0; i < task.hills.length; i++) {
				Hill hill = task.hills[i];
				for (int j = 0; j < nodes.length; j++) {
					Node node = nodes[j];
					if (node.contains(hill.x0, hill.y0)) {
						node.add(hill);
					}
				}
			}

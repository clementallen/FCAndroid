...
public class Cloud implements CameraSubject, ClockObserver, LiftSource {
	...
	public Cloud(XCModelViewer xcModelViewer, float x, float y, float size, float lifeSpan, float age, boolean blueThermal, float h) {
		...
		myRnd = (float) (Tools3d.rnd(random, 0, 1));
	...
	private static final int MAX_NODES = 5; // register with upto 3 overlapping
											// nodes
			...
			for (int i = 0; i < nodes.length; i++) {
				if (nodes[i].contains(x, y)) {
					...
					if (next >= MAX_NODES) { // TODO: tune max nodes
						Log.i("FC", "Cloud over too many nodes !");
						this.asString();
						break;
					}
	...
	public boolean contains(float[] p) {
		...
		// within bounding box ?
		if (p[0] > cx + thermalRadius)
			return false;
		if (p[0] < cx - thermalRadius)
			return false;
		if (p[1] > cy + thermalRadius)
			return false;
		if (p[1] < cy - thermalRadius)
			return false;
	...
	public void tick(float t, float dt) {
		...
		if (lifeCycle.notBorn()) {
			Log.w("FC Cloud", "Destroy notBorn");
			destroyMe();
			return;
		}
	...
	public float[] getEye() {
		if (myRnd > 0.5) {
			...
			return new float[] { x, y - (2 + size * 2), h / 2 };
		}
	}
	...
	public float getLift(float[] p) {
		...
		if (r >= thermalRadius)
			return 0;
		float bp = p[2] / h; // base proximity
		float bp_factor = (float) (1 - Math.pow(bp, 1000));
		// Log.i("FC Cloud", "bp_factor:" + bp_factor+" "+bp+" "+p[2]);
		...
		if (r > coreRadius) {
			return liftMax_ * (1 - (r - coreRadius) / (thermalRadius - coreRadius)) * bp_factor;
		} else {
			// we are in the core
			return liftMax_ * bp_factor;
		}
	}
	...
	public void setWind() {
		 ...
		 */
		// float WIND_MAX =
		// xcModelViewer.xcModel.gliderManager.gliders[0].getSpeed(0); // no
		// gliders yet !
		float WIND_MAX = 1.0f;
	 ...
	 * </pre>
	 * 
	 * And map the verticies onto a hemi-sphere to get a cloud like shape:
	 * 
	 * <pre>
	 * 
	 * 	         p7  .---.  p4
	...
	public class Shape3d {
		...
		private void setAngles() {
			...
			for (int quad = 0; quad < 4; quad++) {
				...
				theta[quad] = Tools3d.rnd(random, lower, upper);
			}

			lower = 0.4 * Math.PI / 2;
			upper = 0.7 * Math.PI / 2;
			for (int quad = 0; quad < 4; quad++) {
				landa[quad] = Tools3d.rnd(random, lower, upper);
			}
	...
	void asString() {
		Log.i("FC", "Cloud(" + myID + "): size = " + size + "x=" + Tools3d.round(x) + ", y=" + Tools3d.round(y) + ", age=" + Tools3d.round(lifeCycle.age)
				+ ", nNodes=" + numNodes);
	}
}

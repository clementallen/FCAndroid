...
public class Balloon extends GliderType {
	...
	try {
	    p.writeFile();
	    Log.i("FC", "Wrote the data file for the Balloon.");
	} catch (IOException e) {
	    Log.i("FC", "Error: " + e.toString());
	}
    }
}
...
class Balloon3d extends Obj3dDir {
	...
	this.lathePolygons(new float[][] {{0,0.1f/2,0.11f},{0, 0.5f/2, 0.7f/2},{0,0.75f/2,1.2f/2},{0,0.5f/2,1.7f/2},{0,0.00001f, 1.8f/2}}, new int[] {Color.WHITE, Color.GRAY}, 8);

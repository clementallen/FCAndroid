...
public class Sailplane extends GliderType {
	...
	public static void main(String[] args) {
		...
		try {
			s.writeFile();
			Log.i("FC", "Wrote the data file for the sailplane.");
		} catch (IOException e) {
			Log.i("FC", "Error: " + e);
		}
	}
...
class Sailplane3d extends Obj3dDir {
	...
	private void init() {
		...
		double R = 5.0;
		float h1 = (float) (R * (1 - Math.cos(1.0 / R)));// bend of inner wing
															// panels due to
															// weight
		float h2 = (float) (R * (1 - Math.cos(2.0 / R)));// bend of outer wing
															// panels due to
															// weight
		...
		// this.addPolygon2(new float[][] {{-tspan, y1, 0}, {tspan, y1, 0},
		// {tspan, y2, 0}, {-tspan, y2, 0}});
		...
		// this.addPolygon2(new float[][] {{0, y1, 0}, {0, 0, -0.1f}, {0, 0,
		// 0}});

		this.scaleBy(scaleModel);
		this.translateBy(0, 0, 0.015f - (at * scaleModel), false);
